...
def make_uci_day_one_frame():
    path = PACKAGE / "data" / "raw" / "uci_student_dropout.csv"
...
def paired_cluster_test(first, second, comparison, seed):
    first = first.set_index(KEY_COLUMNS).sort_index()
    second = second.set_index(KEY_COLUMNS).sort_index()
    common = first.index.intersection(second.index)
    first = first.loc[common].reset_index()
    second = second.loc[common].reset_index()

    if not first[KEY_COLUMNS].equals(second[KEY_COLUMNS]):
        raise ValueError("Paired test rows are not aligned.")
    if not np.array_equal(
        first["y_true"].to_numpy(),
        second["y_true"].to_numpy(),
    ):
        raise ValueError("Paired test targets are not aligned.")

    y_true = first["y_true"].to_numpy()
    first_score = first["score"].to_numpy()
    second_score = second["score"].to_numpy()
    group_codes, student_ids = pd.factorize(
        first["id_student"],
        sort=True,
    )
    ...
    observed = {
        name: function(y_true, second_score)
        - function(y_true, first_score)
    ...
    for _ in range(BOOTSTRAP_SAMPLES):
        ...
        row_weights = group_weights[group_codes]
        if (
            row_weights[y_true == 1].sum() == 0
            or row_weights[y_true == 0].sum() == 0
        ):
            continue
        for name, function in metric_functions.items():
    ...
    extreme_counts = {
        name: 0 for name in metric_functions
    }
    for _ in range(PERMUTATION_SAMPLES):
        ...
        swap_row = swap_student[group_codes]
        permuted_first = np.where(
            swap_row,
            second_score,
            first_score,
        )
        permuted_second = np.where(
            swap_row,
            first_score,
            second_score,
        )
        for name, function in metric_functions.items():
            permuted_difference = (
                function(y_true, permuted_second)
                - function(y_true, permuted_first)
            )
            if abs(permuted_difference) >= abs(observed[name]):
                extreme_counts[name] += 1

    rows = []
    for name, values in bootstrap_values.items():
        lower, upper = np.percentile(values, [2.5, 97.5])
        p_value = (
            1 + extreme_counts[name]
...
def paired_bootstrap_controlled(predictions):
    ...
    for index, (earlier, later) in enumerate(
        ...
        first = xgb.loc[xgb["cutoff_day"] == earlier]
        second = xgb.loc[xgb["cutoff_day"] == later]
        rows.extend(
            paired_cluster_test(
                first,
                second,
                f"day_{later}_minus_day_{earlier}",
...
def paired_model_comparisons(predictions):
    comparisons = (
        ("uci_day_one", 0),
        ("dynamic_landmark", 35),
        ("presentation_holdout_2014J", 75),
    )
    rows = []
    for index, (cohort, cutoff) in enumerate(comparisons):
        frame = predictions.loc[
            (predictions["cohort"] == cohort)
            & (predictions["cutoff_day"] == cutoff)
        ]
        logistic = frame.loc[
            frame["model"] == "Logistic Regression"
        ]
        xgboost = frame.loc[frame["model"] == "XGBoost"]
        rows.extend(
            paired_cluster_test(
                logistic,
                xgboost,
                f"{cohort}_xgboost_minus_logistic",
                RANDOM_STATE + 900 + index,
            )
        )
    return pd.DataFrame(rows)
...
def main():
    ...
    print("Step 8: Compare XGBoost with logistic regression")
    model_comparisons = paired_model_comparisons(predictions)
    model_comparisons.to_csv(
        RESULTS / "journal_model_comparisons.csv",
        index=False,
    )

    print("Step 9: Save journal figures")
